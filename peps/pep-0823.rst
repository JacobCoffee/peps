...
It defines ``PY_NO_ANALYTICS``, an environment variable that package tools
...
* First, it would break existing analytics infrastructure that
  indexes and maintainers rely on. Download counts, geographic distribution,
  abuse detection.
* Second, most users genuinely don't care about aggregate
...
* Third, opt out matches how this works elsewhere. Browser Do Not Track works
  the same way.
* Fourth, opt-in data is far less valuable. Analytics companies
  want complete datasets. If only 2% of users opt in, the data reveals almost
  nothing about real-world usage patterns. Opt-out gives them the coverage they
  need while still letting privacy-conscious users decline.
...
Python already does this. ``PYTHONDONTWRITEBYTECODE`` (also ``python -B``)
stops .pyc files. ``PIP_NO_CACHE_DIR`` (also ``pip --no-cache-dir``) disables
pip's cache. Same pattern.
...
that without changing the :ref:`packaging:simple-repository-api`. It works for HTML responses
...
Package installation tools SHOULD recognize ``PY_NO_ANALYTICS``. When set
to any nonempty value (``1``, ``true``, ``yes``, etc.), the tool SHOULD
signal the opt-out to package indexes. An empty value (``PY_NO_ANALYTICS=``)
or unset variable means analytics are allowed.

Tools SHOULD also recognize ``INDEX_NO_ANALYTICS`` as an alias.

Example:

.. code-block:: bash

    export PY_NO_ANALYTICS=1
...
.. code-block:: bash
    ...
    PY_NO_ANALYTICS=1 pip install requests
...
* Return the package normally.
* Leave this request out of data sent to third-party analytics.
* Still use the request for abuse prevention, rate limiting, operational
  monitoring, and legal compliance like  OFAC screening <https://ofac.treasury.gov/>__`.
...
Tool maintainers SHOULD include this in their next release. Tools that maintain
multiple release branches SHOULD backport to currently supported versions where
feasible. The implementation is small (check an env var, add a header) and
carries no risk of breaking existing behavior.
...
.. code-block:: python
    ...
    def wants_analytics_opt_out():
        for var in ('PY_NO_ANALYTICS', 'INDEX_NO_ANALYTICS'):
...
don't care enough to opt in. Opt-out matches industry practice. Also, opt-in
data has little value—analytics companies need complete datasets, not the 2%
who bother clicking a checkbox.
